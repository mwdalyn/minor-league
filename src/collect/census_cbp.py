...
def get_state_fips(state, cbp_year=2023):
	...
	if state is None:
		...
		return None
	r = requests.get("https://api.census.gov/data/{}/cbp?get=NAME&for=state:*".format(str(acs5_year)))
	r.raise_for_status()
...
def get_city_fips(city, state, cbp_year=2023):
	...
	if len(matches)==1:
		city_fips = matches[0][2] # Return cbp place sublist then select city_fips element [2]
